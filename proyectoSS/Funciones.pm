...
sub obtiene_incidentes
{
        ...
        my $id=$argumentos[2];
        my $contador;
        my $UF_Data = openSnortUnified($file) or die "ERROR al abrir archivo"; #abre el archivo unified2
		...
		#	print Dumper($record);
                        ...
                        {
                                my $paquete;
                                #si el incidente ya existe en el hash de eventos se agrega 1 al contador y se sobreescribe registro ultimo
...
#        print "\n\nentro a lote sdsd";
        print "\n\nprocesando archivo";
        my $referenciaarch = shift; #obtendra la referencia al archivo y guardara ese valor en esta variable
      
#### no entiendo esta linea : my @files = @{$referenciaarch};
        my @files = @{$referenciaarch}	; 
        my $directory_log = shift;   #obntendra algo de un directorio y lo almacenara en esta variable
       #print @files;
        #print "entro\n";
        #my $directory_log = $argumentos[1];
        #my $directory = $argumentos[2];
        ...
        open (STDERR, '>>', $directory_log.'bitacora.log') or die "No se pudo abrir";       # se manejan los errores, se supone que si no se puede abrir un archivo
												#se envia a la bitacora
	#se definen variables
        my $incidentes_ref;
        my @resultado;
        my %incidentes=(); #se sefine un hash vacio
        my $id=0;
        my $posicion_final;
                #recorre el arreglo y obtiene incidentes de cada archivo
                foreach(@files)
                {
                        print "\nprocesando $_\n";
                       @resultado=obtiene_incidentes(\%incidentes,$_,$id); #se van añadiendo los valores que se obtienen al llamar la funcion obtiene_incidentes
									   #a la cual se la pasa un hash
                       $id=$resultado[1]; #se asigna el indice n°1 del arreglo resultado a la variable id
                       %incidentes=%{$resultado[0]};  
        ...
        return \%incidentes;   #regresa un valor que es la referencia al hash
...
sub imprime_incidentes
{
	...
	#		print  $salida_plano Dumper ($incidentes{$key}{'eventos'}); 
...
###############ordenamiento
			...
			#$str =~  s/[^0-9]//g;
			#print $str;
			#print $salida_plano Dumper ($incidentes{$key}{'eventos'}) ; # =~ s/[^0-9]//g; 
		
#			print $salida_plano , "\n";
        }
        print "llego aqui\n";
				#print Dumper ($incidentes{'10000001\^\\fe80:0:0:0:222:15ff:fed3:77fc\^\\58'});
